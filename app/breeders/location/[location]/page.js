...
export default async function LocationPage({ params }) {
  ...
  const locationBreeders = breeders
    .map((b) => ({
      ...b,
  ...
  const breedsInLocation = [
    ...new Set(locationBreeders.flatMap((b) => b.breeds || [])),
  ...
  const structuredData = breadcrumbSchema([
    ...
    {
      name: locationName,
      url: `https://breedwise.co.uk/breeders/location/${params.location}`,
    },
  ]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 md:px-8">
      ...
      <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="mb-3 text-xs uppercase tracking-[0.3em] text-slate-500">
          Breeds available in {locationName}
        </p>
        <div className="flex flex-wrap gap-2">
          {breedsInLocation.map((breed) => (
            <Link
              key={breed}
              href={`/breeders/${slugify(breed)}/${params.location}`}
