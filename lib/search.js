...
async function fetchBreederIdsForFilter(supabase, { animal, breeds }) {
  ...
  while (true) {
    ...
    if (breeds.length > 0) {
      // Partial match so "Labrador" finds "Labrador Retriever"
      const clauses = breeds
        .map((b) => String(b || "").trim().replace(/[%_(),&]/g, ""))
        .filter(Boolean)
        .map((safe) => `breed.ilike.%${safe}%`);
      if (clauses.length === 0) break;
      breedQuery = breedQuery.or(clauses.join(","));
    }
