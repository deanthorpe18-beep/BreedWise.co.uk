...
const LIGHT_SELECT =
  "id, slug, name, town, county, postcode, lat, lng, membership_tier, claimed_at, is_featured, google_rating, google_review_count, council_licence, kennel_club, health_testing, licence_verified, availability_status, status, hero_image_url, phone, website, featured_until";
...
function sortLightRows(rows, sortBy) {
  const sorted = [...rows];
  switch (sortBy) {
    case "distance":
...
export async function searchBreeders({
  ...
  lightRows = sortLightRows(lightRows, sortBy);
