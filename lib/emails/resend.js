...
import { buildClaimPath, outreachClaimPath, outreachSignupPath, outreachWelcomePath, outreachProfilePath } from "@/lib/breeder-onboarding";
...
const contactFromEmail = "info@breedwise.co.uk";

function getFromHeader() {
  const raw = (process.env.RESEND_FROM_EMAIL || contactFromEmail).trim();
  let email = raw.includes("<") ? raw.match(/<([^>]+)>/)?.[1]?.trim() : raw;
  if (!email || email.toLowerCase().includes("noreply@")) {
    email = contactFromEmail;
  }
  return email.includes("<") ? email : `BreedWise <${email}>`;
}

function getReplyTo() {
  return adminEmail.includes("@") ? adminEmail : contactFromEmail;
}
...
async function sendEmail(payload) {
  ...
  // rate limit, invalid recipient); it returns { data, error }. Surface it
  // so callers can record accurate delivery status instead of false "sent".
  if (result?.error) {
    const err = result.error;
    throw new Error(err.message || err.name || "Resend send failed");
  }
  return result;
}
...
export async function sendVerificationEmail(to, verificationUrl) {
  ...
  return sendEmail({ from: getFromHeader(), to, subject, html, text });
}
...
Review in admin: ${adminUrl}${plainTextFooter()}`;
  ...
  return sendEmail({ from: getFromHeader(), to: adminEmail, subject, html, text });
}
...
export async function sendClaimInvitation(to, breederName, breederSlug) {
  const siteUrl = getSiteUrl();
  const welcomeUrl = `${siteUrl}${outreachWelcomePath(breederSlug, breederName)}`;
  ...
  const profileUrl = `${siteUrl}${outreachProfilePath(breederSlug)}`;
  ...
  const html = `<div style="font-family:Inter,system-ui,sans-serif;max-width:480px;margin:0 auto;color:#2D3436;">
    ...
    <p style="margin:16px 0;"><a href="${profileUrl}" style="color:#00BFA5;text-decoration:none;font-weight:600;">View your public profile →</a></p>
    ...
    <p style="margin:24px 0;"><a href="${welcomeUrl}" style="display:inline-block;background:#00BFA5;color:#fff;text-decoration:none;padding:12px 24px;border-radius:9999px;font-weight:600;">See your listing &amp; claim it</a></p>
...
See your listing and claim it: ${welcomeUrl}

Already have a BreedWise account? Claim your listing: ${claimUrl}

If you have any questions, reply to this email or contact us at info@breedwise.co.uk.

---
BreedWise | breedwise.co.uk`;

  const result = await sendEmail({ from: getFromHeader(), to, subject, html, text });
