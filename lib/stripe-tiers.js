...
export const DEFAULT_TIER_CONFIG = {
  bronze: {
    ...
    photoLimit: 5,
    searchPriority: 1,
    features: TIER_CONFIG.bronze.features,
  },
  silver: {
    name: "Silver",
    monthlyPrice: 7.99,
    photoLimit: 10,
    searchPriority: 2,
    features: TIER_CONFIG.silver.features,
  },
  gold: {
    name: "Gold",
    monthlyPrice: 9.99,
...
export async function getTierConfigFromDB(tier) {
  ...
  return {
    ...
    // Prefer code config so marketing copy always matches gated features
    features: TIER_CONFIG[tier]?.features || (Array.isArray(data.features) ? data.features : JSON.parse(data.features)),
...
export async function getAllTiersFromDB() {
  ...
  return data.map((row) => ({
    ...
    features: TIER_CONFIG[row.tier]?.features || (Array.isArray(row.features) ? row.features : JSON.parse(row.features)),
