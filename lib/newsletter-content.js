...
const BUYER_TIPS = [
  { title: "Always visit in person", body: "Never buy a pet without meeting the breeder and seeing the mother with her litter. Video calls are not enough.", link: "/education/red-flags", color: "#FF6B6B" },
  { title: "Ask about health testing", body: "Responsible breeders health-test parent dogs for breed-specific conditions. Ask to see certificates — don't take their word for it.", link: "/education/health-testing", color: "#00BFA5" },
  { title: "Check the council licence", body: "UK breeders producing 3+ litters a year need a council breeding licence. Ask for the licence number and verify it.", link: "/education/what-to-ask", color: "#0ea5e9" },
  { title: "Use our viewing checklist", body: "Print our puppy viewing checklist and take it with you. It covers environment, paperwork, and warning signs.", link: "/guides/puppy-viewing-checklist", color: "#FFB545" },
  { title: "Compare before you commit", body: "Save up to 3 breeders on BreedWise and compare them side by side — location, reviews, licences, and health info.", link: "/account/compare", color: "#6366f1" },
  { title: "Watch for red flags", body: "Multiple breeds always available, pressure to pay quickly, no mother present, or meeting in a car park — walk away.", link: "/education/red-flags", color: "#dc2626" },
];
...
function btn(url, label, bg = "#00BFA5") {
  return `<table cellpadding="0" cellspacing="0" style="margin:16px 0 0;"><tr><td style="border-radius:9999px;background:${bg};"><a href="${url}" style="display:inline-block;padding:12px 28px;color:#fff;font-weight:700;font-size:14px;text-decoration:none;font-family:Inter,system-ui,sans-serif;">${label}</a></td></tr></table>`;
}

function card(heading, body, opts = {}) {
  const { color = "#00BFA5", link, image, imageAlt } = opts;
  const imgBlock = image
    ? `<img src="${image}" alt="${imageAlt || heading}" width="100%" style="display:block;width:100%;max-width:552px;height:auto;border-radius:12px 12px 0 0;margin:0;" />`
    : "";
  const linkBlock = link ? btn(`${SITE}${link.url}`, link.label, color) : "";
  return `<table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:20px;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0;background:#fff;">
    ${imgBlock ? `<tr><td style="padding:0;line-height:0;">${imgBlock}</td></tr>` : ""}
    <tr><td style="padding:20px 24px;border-left:4px solid ${color};">
      <h3 style="margin:0 0 8px;font-size:17px;color:#0f172a;font-family:Inter,system-ui,sans-serif;">${heading}</h3>
      <p style="margin:0;color:#475569;line-height:1.65;font-size:14px;font-family:Inter,system-ui,sans-serif;">${body}</p>
      ${linkBlock}
    </td></tr>
  </table>`;
}
...
function tipBox(title, body, color = "#FFB545") {
  return `<table cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:20px;"><tr><td style="background:${color}15;border:1px solid ${color}40;border-radius:16px;padding:20px 24px;">
    <p style="margin:0 0 6px;font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${color};font-family:Inter,system-ui,sans-serif;">💡 Tip</p>
    <p style="margin:0 0 6px;font-size:16px;font-weight:700;color:#0f172a;font-family:Inter,system-ui,sans-serif;">${title}</p>
    <p style="margin:0;color:#475569;line-height:1.65;font-size:14px;font-family:Inter,system-ui,sans-serif;">${body}</p>
  </td></tr></table>`;
}
...
function wrapEmail({ title, subtitle, accentColor, heroImage, heroAlt, sections, badge }) {
  const heroBlock = heroImage
    ? `<img src="${heroImage}" alt="${heroAlt || title}" width="600" style="display:block;width:100%;max-width:600px;height:220px;object-fit:cover;" />`
    : "";

  return `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f1f5f9;">
<div style="font-family:Inter,system-ui,sans-serif;max-width:600px;margin:0 auto;background:#f1f5f9;">
  <table cellpadding="0" cellspacing="0" width="100%" style="background:linear-gradient(135deg,${accentColor},${accentColor}dd);">
    <tr><td style="padding:28px 24px;text-align:center;">
      <img src="${LOGO}" alt="BreedWise" width="48" height="48" style="display:inline-block;border-radius:12px;background:#fff;padding:4px;" />
      <p style="margin:12px 0 4px;font-size:11px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:rgba(255,255,255,0.85);">${badge || "BreedWise Newsletter"}</p>
      <h1 style="margin:0;font-size:26px;color:#fff;line-height:1.25;">${title}</h1>
      ${subtitle ? `<p style="margin:10px 0 0;font-size:15px;color:rgba(255,255,255,0.9);line-height:1.5;">${subtitle}</p>` : ""}
    </td></tr>
  </table>
  ${heroBlock}
  <div style="padding:20px 16px 32px;">
    ${sections.join("")}
    <table cellpadding="0" cellspacing="0" width="100%"><tr><td style="padding:20px 8px;text-align:center;border-top:1px solid #e2e8f0;">
      <p style="margin:0;font-size:13px;color:#64748b;">BreedWise — UK pet breeder directory</p>
      <p style="margin:8px 0 0;"><a href="${SITE}" style="color:#00BFA5;font-weight:600;text-decoration:none;">breedwise.co.uk</a></p>
      <p style="margin:12px 0 0;font-size:11px;color:#94a3b8;">Dogs, cats, birds, fish, reptiles &amp; small pets. Directory only — we do not sell animals.</p>
    </td></tr></table>
  </div>
</div></body></html>`;
}
...
async function fetchSiteData(adminClient) {
  ...
  const [
    ...
    adminClient.from("breeders").select("*", { count: "exact", head: true }).eq("status", "claimed_profile"),
...
function buildWeekly(data) {
  ...
  const sections = [
    card("This week on BreedWise", `${data.totalBreeders.toLocaleString()} breeder listings across the UK — dogs, cats, birds, fish, reptiles and small pets. <strong>${data.newBreeders}</strong> new listings added in the last 7 days and <strong>${data.claimedCount}</strong> breeders have claimed their profiles.`, {
      color: "#00BFA5",
...
function buildBuyerTips(_data) {
  ...
  const sections = uniqueTips.map((t) =>
    card(t.title, t.body, { color: t.color, link: { url: t.link, label: "Read the full guide" } })
  );
  sections.push(card("Free breeder checklist", "Download our printable checklist — health tests, licences, paperwork, and red flags to watch for before you hand over any money.", {
    color: "#6366f1",
  ...
  return {
    subject: "Stay safe when choosing a breeder — BreedWise buyer guide",
    ...
    subtitle: "Protect yourself before you make contact",
    accentColor: "#FF6B6B",
...
function buildFeaturedBreeders(data) {
  ...
  const sections = [
    card("Gold featured breeders", "These breeders have upgraded to Gold membership for priority placement. Each has claimed and verified their profile on BreedWise.", {
      color: "#FFB545",
...
function buildCompareTool(data) {
  const sections = [
    ...
    card("How to compare wisely", "Look for claimed profiles, council licences, health test info, and consistent Google reviews. Our education hub walks you through every step.", {
      color: "#00BFA5",
